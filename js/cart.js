...
(function (G) {
  ...
  function state() {
    if (!data) {
      data = read();
      /* An older cart may still hold the gift box, which the shop no longer
         sells. Drop it once and write the cart back so it stays gone. */
      var kept = data.items.filter(function (i) { return i.productId !== 'gift-box'; });
